...
with open(os.path.join(data_dir, "words_alpha.txt")) as f:
    word_list = [w.strip().upper() for w in f.readlines()]
prefix_tree = PrefixTree(word_list)

for decoder in ["best_path", "word_beam_search"]:
    for img_filename in Path(data_dir).files("*.png"):
        print(f"Reading file {img_filename} with decoder {decoder}")

        # read text
        img = cv2.imread(img_filename, cv2.IMREAD_GRAYSCALE)
        scale = (
            sample_config[img_filename.basename()]["scale"]
            if img_filename.basename() in sample_config
            else 1
        )
        margin = (
            sample_config[img_filename.basename()]["margin"]
            if img_filename.basename() in sample_config
            else 0
        )
